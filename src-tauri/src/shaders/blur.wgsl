// Blur parameters are per-tile to avoid sampling outside the valid input region.
// Padding keeps std140-like alignment for uniform buffers.
struct BlurParams {
    radius: u32,
    tile_offset_x: u32,
    tile_offset_y: u32,
    // Per-tile dimensions for bounds checks and clamping.
    tile_width: u32,
    tile_height: u32,
    _pad0: u32,
    _pad1: u32,
...
// Conservative clamp for rgba16float to avoid INF/NaN from extreme values.
...
// Unnormalized Gaussian weight for distance-based blur.
fn gaussian(x: f32, sigma: f32) -> f32 {
    return exp(-(x * x) / (2.0 * sigma * sigma));
}

// Horizontal pass uses a bilateral-like weight: Gaussian distance * luma similarity.
@compute @workgroup_size(256, 1, 1)
fn horizontal_blur(@builtin(global_invocation_id) id: vec3<u32>) {
    let tile_dims = vec2<u32>(params.tile_width, params.tile_height);
    // Guard against over-dispatch on either axis.
    ...
    // Map tile-local coordinates into the full input texture.
    ...
    // Clamp center sample to valid input bounds.
    let clamped_center_coord = clamp(
        vec2<i32>(absolute_coord),
        vec2<i32>(0),
        full_dims - 1
    );
    let center_color = clamp(
        textureLoad(input_texture, vec2<u32>(clamped_center_coord), 0).rgb,
        vec3(0.0),
        vec3(F16_MAX)
    );
    ...
    // Sample a 1D window along X, clamped to the tile's input bounds.
    ...
    // Normalize weighted sum; fallback to center if weights collapse.
...
// Vertical pass mirrors horizontal and clamps to the per-tile valid region.
...
fn vertical_blur(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // Clamp sampling to the tile region to avoid stale ping-pong data.
    // Clamp sampling to the tile region to avoid stale ping-pong data.
    let input_dims = vec2<i32>(i32(params.tile_width), i32(params.tile_height));
    ...
    // Sample a 1D window along Y, clamped to the tile's input bounds.
    for (var offset = -radius; offset <= radius; offset = offset + 1) {
        let sample_coord = clamp(
